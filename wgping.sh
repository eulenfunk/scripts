...
if [ "$ipfail" == "false" ] ; then
  [ -f $gwlostcount.* ] && logger -s wgping "OK on $host out of $checkhosts, deleting gwlostcount" && rm -f $gwlostcount.* 2>/dev/null
  ...
  logger -s wgping "none of $chechosts longer pingable, lostcount=$newnum"
  if [ $onisland -eq 0 ] ; then  # dont start action if we have never seen the world since reboot
    if [ $newnum -eq 4 ] ; then
      [ -f $upgrade_started ] && exit
      logger -s wgping "rebooting"
      sleep 10
      reboot
    elif [ $newnum -eq 3 ] ; then
      logger -s wgping "restarting networking and firewall"
      ifconfig $wanif down
      sleep 2
      ifconfig $wanif up
      sleep 2
      ...
      logger -s wgping "ifupdown $wanif"
      ifdown $wanif 
      sleep 2
      ifup $wanif
      sleep 2
